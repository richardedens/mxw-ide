...
(function (root, factory) {
    ...
    $.fn.extend({
        ...
        /* MXW - Interaction from modules and controller */
        ...
        actions: function(){
            ...
            {
                var controller = $(element).closest('[data-mxw-controller]').data('mxw-controller');
                var module = $(element).closest('[data-mxw-module]').data('mxw-module');
            ...
            this.each(function(){
                $(this).on('click', function(event){
                    var tabAction = $(this).data('mxw-action'),
                        controller = $(this).closest('[data-mxw-controller]').data('mxw-controller'),
                        module = $(this).closest('[data-mxw-module]').data('mxw-module');
        ...
        /* MXW - Error handlers to handle each error from moonlight UI. */
        ...
        view: function(name, view, render) {
            ...
            vw.renderCached = function(cb) {
                ...
                modules[module].views[name].__container = $(modules[module].views[name].container);
                modules[module].views[name].__container.html('<div class="col mxw-progress-indicator progress-indicator s12 m12 l12"><div class="progress"><div class="indeterminate"></div></div></div>');
                if (typeof cb === "undefined") {
                    modules[module].views[name].__container = $(modules[module].views[name].container);
                    modules[module].views[name].__container.html(modules[module].views[name].__cached);
                    if (modules[module].views[name].__initialized === true) {
            ...
            vw.render = function(cb, postParams, getParams) {
                ...
                if (vw.__usecached === true && modules[module].views[name].__cached !== '') {
                    ...
                    modules[module].views[name].__container = $(modules[module].views[name].container);
                    if (typeof $(modules[module].views[name].__container).children('.mxw-buffer.mxw-buffer-hide')[0] === 'undefined') {
                        $(modules[module].views[name].__container).html('<div class="mxw-buffer mxw-buffer-one mxw-buffer-show"></div><div class="mxw-buffer mxw-buffer-two mxw-buffer-hide"></div>');
                    }
                    if (typeof cb === "undefined") {
                        modules[module].views[name].__container = $(modules[module].views[name].container);
                        $(modules[module].views[name].__container).children('.mxw-buffer-hide').html(modules[module].views[name].__render(modules[module].views[name].__template));
                        var bufferShow = $(modules[module].views[name].__container).children('.mxw-buffer-show');
                        var bufferHide = $(modules[module].views[name].__container).children('.mxw-buffer-hide');
                        $(bufferShow).removeClass('mxw-buffer-show').addClass('mxw-buffer-hide');
                        $(bufferHide).removeClass('mxw-buffer-hide').addClass('mxw-buffer-show');
                        ...
                        if (typeof $('.mxw-progress-indicator-container .mxw-progress-indicator')[0] === 'undefined') {
                            $('.mxw-progress-indicator-container').append('<div class="col mxw-progress-indicator progress-indicator s12 m12 l12"><div class="progress"><div class="indeterminate"></div></div></div>');
                        }
                        $(modules[module].views[name].__container).children('.mxw-buffer-show').append('<div class="mxw-progress-overlap"></div>');
        ...
        model: function(name, model) {
            ...
            function searchFor(param, key, nameOfObject) {
                var value = param[key];
                $('[data-mxw-module="' + mdl.__module + '"').find('[data-mxw-model="' + mdl.__name + '.' + nameOfObject + '.' + key + '"]').each(function () {
                    if ($(this).is(":checkbox")) {
                        if (value === 1) {
            ...
            mdl.removeError = function() {
                ...
                $('[data-mxw-module="' + module+ '"]').find('[data-mxw-error="' + name + '.error"]').each(function() {
                    if ($(this).is( "input" ) ||
                        $(this).is( "textarea" ) ||
                        $(this).is( "select" ) ||
                        $(this).is( "checkbox" ) ||
            ...
            mdl.set = function(param, value) {
                ...
                if (typeof param === 'object') {
                    ...
                    mdl[param] = value;
                    $('[data-mxw-module="' + mdl.__module + '"').find('[data-mxw-model="' + mdl.__name + '.' + param + '"]').each(function () {
                        if ($(this).is(":checkbox")) {
                            if (value === 1) {
                                $(this).prop('checked', true);
                            } else {
            ...
            mdl.__broadcast = function(model, param, evt){
                ...
                $('[data-mxw-module="' + module + '"]').find('[data-mxw-model="' + model + '.' + param + '"]').each(function() {
                    var modelParameter = [];
                    if (param.indexOf('.') !== -1) {
            ...
            mdl.__initTwoWayBinding = function(){
                ...
                $('[data-mxw-module="' + module+ '"]').find('[data-mxw-model*="' + name + '."]').each(function(){
                    if ($(this).data('mxw-model').indexOf('.') !== -1) {
                        var modelParameter = $(this).data('mxw-model').split('.'),
                        ...
                        console.warn(labelLib + 'You must specify a model and its parameter (example "modelName.param") in the mxw-model attribute. I got: ' + $(this).data('mxw-model') + ' in module "' + module + '"');
                    }
                });
            };
        ...
        deenergize: function(element) {
            ...
            $(element).find('[data-mxw-action]').off();

            /* Detach all events */
            $(element).off();

            /* Detach two-way databinding */
            $(element).find('[data-mxw-model]').off();
        ...
        energize: function(element) {
            ...
            $(element).find('[data-mxw-action]').actions();
    ...
    window.$mxw = window.mxw = window.$ml = $.noConflict();
}));